...
def linear_test(config, model, datasets):
    ...
    for i in tqdm(range(ceil(num_instance / config.eval_batch_size))):
        ...
        if k > 0:
            ...
            top_k_scores[slice] = np.take_along_axis(preds, top_k_idx[slice], axis=1)
...
def linear_run(config):
    ...
    if config.test_file is not None:
        metric_dict, top_k_idx, top_k_scores = linear_test(config, model, datasets)
        ...
        if config.save_k_predictions > 0:
            ...
            with open(config.predict_out_path, "w") as fp:
                for idx, score in zip(top_k_idx, top_k_scores):
                    out_str = " ".join([f"{classes[i]}:{s:.4}" for i, s in zip(idx, score)])
                    fp.write(out_str + "\n")
